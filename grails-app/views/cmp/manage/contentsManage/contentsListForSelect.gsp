...
<head>
    ...
    <script type="text/javascript">
        $(function(){
            <g:if test="${params.selectType=="one"}">
                    $("input[name='ids']").click(function(){
                        if($(this).attr("checked")){
                         parent.selectContentsInfo($(this).parent().find("input[name='title']").val(),$(this).val());
                        }
                    });
                    </g:if>
                    <g:else>
            ...
            $("#selectBt").click(function(event){
                //alert("delete");
                var isSelected=false;
                for(i=0;i<$("input[name='ids']").length;i++){
                    if($($("input[name='ids']")[i]).attr("checked")){
                        parent.selectContentsInfos($($("input[name='ids']")[i]).parent().find("input[name='title']").val(),$($("input[name='ids']")[i]).val());
                        isSelected=true;
                    }
                }
                if(!isSelected){
...
<div style="margin: 10px;">
    ...
    <!--搜索条件-->
    <div>
        <g:form name="searchForm" action="contentsListForSelect" class="form-horizontal">
            <input type="hidden" value="${params.selectType?:"one"}" name="selectType"/>
            <div class="row">
    ...
    <g:if test="${contentsInfoList}">
        <div>
                ...
                <g:form name="form1" action="contentsDelete" params="${params}">
                    ...
                    <g:each in="${contentsInfoList}" status="i" var="contentsInfoInstance">
                        <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
                            <td>
                                <input name="title" type="hidden" value="${contentsInfoInstance.title}"/>
                                <g:if test="${params.selectType=="one"}">
                                    <input type="radio" name="ids" value="${contentsInfoInstance.id}"/>
                                </g:if>
                                <g:else>
            ...
            <div>
                ...
                <g:if test="${params.selectType=="multi"}">
                    <li><label class="checkbox"><input type="checkbox" id="allCheck"><a href="#">全选</a></label></li>
                    <li><button class="button button-primary" id="selectBt">应用</button></li>
            </g:if>
                </ul>
