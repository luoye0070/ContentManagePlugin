package com.lj.cmp.util.common
...
class FormatUtil {
    ...
    //byte数量转换成1024*形式
    public static String byteToExpansion(long byteCount){
        String expansionStr="";
        long byteCountP=byteCount;
        for(int i=0;byteCountP>0;i++){
            long count= byteCountP/1024;
            int byteCountT=byteCountP-count*1024;
            String temp="";
            if(i==0){
                temp=""+byteCountT;
            }else{
            ...
            expansionStr=expansionStr+temp;
            byteCountP=count;
        }
        return expansionStr;
    }
